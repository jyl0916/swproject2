...
#define _DUTY_MIN 10 // servo full clockwise position (0 degree)
#define _DUTY_NEU 10005 // servo neutral position (90 degree)
#define _DUTY_MAX 20000 // servo full counterclockwise position (180 degree
...
void loop() {
  ...
  if(dist_raw <= _TARGET_LOW){
    servo_position = _DUTY_MIN;
    digitalWrite(PIN_LED, LOW);
  } else if ((dist_raw > _TARGET_LOW) && (dist_raw < _TARGET_HIGH)){
    servo_position = map(dist_raw, _TARGET_LOW, _TARGET_HIGH, _DUTY_MIN, _DUTY_MAX);
    digitalWrite(PIN_LED, HIGH);
  } else {
    servo_position = _DUTY_MAX;
    digitalWrite(PIN_LED, LOW);
  }
